...
int read_block(int fd, int blocknum, void* rblock){
	lseek(fd, blocknum*8192, SEEK_SET);
	return read(fd, rblock, 8192);
}
...
static int xmp_getattr(const char *path, struct stat *stbuf)
{
	...
	fseek(disk_fd, 8192* curr_block, SEEK_SET);
	...
	int curr_scan_block = curr_scan_pos/2048;
	int curr_scan_block_pos = curr_scan_pos%2048;
	
	while(curr_scan_pos > 0){
	    fseek(disk_fd,((super->k + 1) * 8192) + curr_scan_pos *64, SEEK_SET);
	    fread(&scan_dir, sizeof(struct directory) , 1, disk_fd);
	    if(scan_dir->file_name == name){
	        break;
	    }else{
	        fseek(disk_fd, (curr_scan_block + 1) * 8192, SEEK_SET);
	        fread(&fatblock_scan, sizeof(struct fatblock),1, disk_fd);  //set scanned directory block
	        curr_scan_pos = fatblock_scan -> fat_entry[curr_scan_block_pos];
	        curr_scan_block = curr_scan_pos/2048;
            curr_scan_block_pos = curr_scan_pos%2048;
	    }
	}
...
//	void* file_block = calloc(sizeof(struct directory),1);
	...
	for(i=1; i < (super->k) + 1; i++) {
		dir_first_fat_pos = i;
		fseek(disk_fd, 8192 * dir_first_fat_pos, SEEK_SET);
	...
	fseek(disk_fd,(super->k +1 * 8192) + (((curr_block * 2048) + curr_block_pos)*64), SEEK_SET);
	...
	new_dir->start_block = ((dir_first_fat_pos-1) * 2048) + (dir_first_free_pos);
	new_dir->flag = 0;
	
	fseek(disk_fd,(super->k * 8192) + (((fat_pos * 2048) + free_pos)*64), SEEK_SET);
